    ...
    <div className="overflow-hidden">
      ...
      <section className="relative h-screen min-h-[600px] flex items-center justify-center pt-20">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <a 
              href="tel:8175891567" 
              className="w-full sm:w-auto bg-safety text-white px-8 py-4 rounded-md font-accent text-xl uppercase tracking-wider hover:bg-safety/90 transition-all transform hover:scale-105 shadow-xl"
            >
      ...
      <div className="bg-safety py-4 relative z-20 pulse-emergency">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-center gap-4 text-white text-center">
          ...
          <a href="tel:8175891567" className="font-heading text-4xl underline decoration-white underline-offset-4">817-589-1567</a>
        </div>
      </div>
      ...
      <section className="bg-safety py-16">
        <div className="container mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <a href="tel:8175891567" className="font-heading text-5xl text-white hover:text-navy transition-colors">817-589-1567</a>
            <Link to="/contact" className="bg-white text-safety px-8 py-3 rounded-md font-accent uppercase tracking-widest font-bold hover:bg-navy hover:text-white transition-all">Schedule Now</Link>
          </div>
        </div>
      </section>
    </div>
...
const ServiceCard: React.FC<{ title: string; desc: string; icon: React.ReactNode; link: string; isUrgent?: boolean }> = ({ title, desc, icon, link, isUrgent }) => (
  <div className={`p-8 rounded-lg shadow-xl transition-all duration-300 transform hover:-translate-y-2 flex flex-col items-center text-center ${isUrgent ? 'bg-safety text-white' : 'bg-white'}`}>
    ...
    <Link 
      to={link} 
      className={`mt-auto px-6 py-2 rounded-md font-accent uppercase tracking-widest font-bold transition-colors ${
        isUrgent 
          ? 'bg-navy text-white hover:bg-white hover:text-navy' 
          : 'bg-safety text-white hover:bg-navy'
      }`}
    >
      {isUrgent ? 'Call Now' : 'Learn More'}
    </Link>
  </div>
);
